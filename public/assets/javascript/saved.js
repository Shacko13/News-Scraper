  ...
  function initPage() {
	...
	function renderNotesList(data) {
	  ...
	  if (!data.notes.length) {
	  	// If we have no notes, display message saying so
	  	currentNote = [
	  	  "<li slass='list-group-item'>",
	...
	function handleArticleNotes() {
	  // This function handles opening the notes modal and displaying out notes
	  ...
	  $.get("/api/notes/" + currentArticle._id).then(function(data) {
	  	// Constructing out initial HTML to add to the notes modal
	  	var modalText = [
	...
	function handleNoteSave() {
	  ...
	  // Setting a variable to hold some formatted data about out note, grabbing the note typed into the input box
	  var noteData;
	  var newNote = $(".bootbox-body textarea").val().trim();
	  // If we actually have data typed into the note input field, format it and post it to the "/api/notes" route and send the formatted noteData as well
	  if (newNote) {
	  	noteData = {
